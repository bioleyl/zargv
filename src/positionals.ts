...
type SplitLastOutput<T extends SplitLastTuple> =
    { [K in T[0][0]]: z.output<T[0][1]> }
    & { [K in T[1][0]]: z.output<T[1][1]> };

type SingleOutput<T extends SingleTuple> =
    { [K in T[0]]: z.output<T[1]> };
...
function parseSplitLastPositionals<T extends SplitLastTuple>(
    tuple: T,
    positionals: string[],
): SplitLastOutput<T> {
...
function splitLast<const T extends SplitLastTuple>(
    tuple: T,
): PositionalsDef<SplitLastOutput<T>> {
...
function parseSinglePositionals<T extends SingleTuple>(
    tuple: T,
    positionals: string[],
): SingleOutput<T> {
...
function single<const T extends SingleTuple>(
    tuple: T,
): PositionalsDef<SingleOutput<T>> {
    const [key] = tuple;
