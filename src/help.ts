...
function buildLeafUsageParts(leaf: LeafCommand<unknown>, options?: LeafHelpRenderOptions): string[] {
  ...
  return [options?.usagePrefix ?? '', hasOptions ? '[OPTIONS]' : '', positionalsDef?._usage ?? ''].filter(Boolean);
}

export function buildLeafUsage(leaf: LeafCommand<unknown>, options?: LeafHelpRenderOptions): string | null {
...
function formatOptionTypeLabel(key: string, kind: OptionKind, choices?: readonly (string | number)[]): string {
  const longFlag = `--${key}`;
  const typeLabel = choices?.length ? `[${choices.join(' | ')}]` : kind === 'boolean' ? '' : `<${kind}>`;
...
function renderLeafOptionLine(flagLabel: string, description: string | undefined, flagColWidth: number): string {
  ...
  return wrapped.map((line, index) => (index === 0 ? `${prefix}${line}` : `${continuation}${line}`)).join('\n');
}

function splitCommandSummaries(commands: Record<string, CommandNode>): {
  ...
  for (const [name, node] of Object.entries(commands)) {
    (node.__brand__ === 'leaf' ? leaves : parents).push({ desc: node.description, name });
  }
...
function buildOptionDescription(
  ...
  const noteParts = [
    ...
    defaultMap?.has(key) && !hasDefaultHint(baseDesc) ? `default: ${formatDefaultValue(defaultValue)}` : '',
...
export function generateHelp(commands: Record<string, CommandNode>, description?: string): string {
  const lines: string[] = [];
...
export function generateLeafHelp(leaf: LeafCommand<unknown>, options?: LeafHelpRenderOptions): string | null {
  const positionalsDef = leaf.positionalsDef as HelpPositionalsDefMeta | undefined;
  if (!leaf.argsDef && !positionalsDef) {
    return null;
  }
  ...
  if (!hasOptions || !describeMap || !parseConfig) {
    return lines.join('\n');
  }
  ...
  for (const [key, describeText] of describeMap.entries()) {
    ...
    optionLines.push({ description, longLabel, shortWithComma });
  }
...
export function printCommandHelp(pathSegments: string[], commands: Record<string, CommandNode>): void {
